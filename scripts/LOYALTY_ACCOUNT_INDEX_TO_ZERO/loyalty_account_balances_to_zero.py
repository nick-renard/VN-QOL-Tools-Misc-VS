...
def run():
    ...
    if choice == '1':
        # Read the JSON file
        with open('scripts/LOYALTY_ACCOUNT_INDEX_TO_ZERO/loyalty_accounts_index.json') as file:
            data = json.load(file)
    elif choice == '2':
        instance = input("Enter the instance: ")
        organization_name = input("Enter the organization name: ")
        with open('scripts/LOYALTY_ACCOUNT_INDEX_TO_ZERO/loyalty_accounts_text.txt') as file:
    ...
    with open('scripts/LOYALTY_ACCOUNT_INDEX_TO_ZERO/postman_request.json', 'w') as file:
